...
export async function assignmentsMenu(): Promise<void> {
  ...
  if (action === 'list') {
    ...
    assignments.forEach((a: any) => {
      ...
      console.log(`  ${chalk.bold(a.id.toString().padEnd(4))} ${a.name.padEnd(40)} [${pub}]`)
    })
    console.log('')
  }
  ...
  if (action === 'show') {
    ...
    console.log(`  Docker Image:${a.dockerImageTag ?? chalk.yellow(' not set')}`)
  ...
  if (action === 'create') {
    ...
    const policyId = await input({ message: 'Submission policy ID (default: 1):', default: '1' })
    ...
    console.log(chalk.green(`✔ Created assignment: ${res.data.name} (ID: ${res.data.id})`))
  }

  if (action === 'update') {
    const id = await input({ message: 'Assignment ID to update:' })
    const name = await input({ message: 'New name (leave blank to keep):' })
    const dockerImage = await input({ message: 'New Docker image tag (leave blank to keep):' })
