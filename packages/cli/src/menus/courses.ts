...
export async function coursesMenu(): Promise<void> {
  ...
  if (action === "list") {
    ...
    courses.forEach((c: any) => {
      console.log(
        `  ${c.id.toString().padEnd(4)} ${c.number.padEnd(10)} ${c.name}`,
      );
    });
    console.log("");
  }
  ...
  if (action === "enrollments") {
    ...
    enrollments.forEach((e: any) => {
      ...
      console.log(
        `  ${e.userId.toString().padEnd(10)}${name.padEnd(30)} ${role}`,
      );
    });
    console.log("");
  }
  ...
  if (action === "enroll") {
    ...
    const res = await api.post(
      `/courses/${courseId}/sections/${sectionId}/enroll`,
      {
    ...
    console.log(
      chalk.green(`✔ User ${userId} enrolled in section ${sectionId}`),
    );
  }
  ...
  if (action === "unenroll") {
    ...
    console.log(
      chalk.green(`✔ User ${userId} removed from section ${sectionId}`),
    );
  }
  ...
  if (action === "create") {
    ...
    console.log(
      chalk.green(`✔ Created course: ${res.data.name} (ID: ${res.data.id})`),
    );
  }
}
